...
def drawPolygon(polygondata, outline, width):
    ...
    for i in range(len(polygondata) - 1):
        id = canvas.create_line(polygondata[i], polygondata[i + 1], width=width, fill=outline,
                                tags=(prefix + str(counter_obj["polygon"])))
...
for curr in avail_colors:
    ...
    counter = counter + 25;
avail_figures = ["freehand", "straight", "square", "circle", "polygon"]
counter = 35
